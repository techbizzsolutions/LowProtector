...
@IonicPage()
@Component({
  selector: 'page-propritorship-first',
  templateUrl: 'propritorship-first.html',
})
export class PropritorshipFirstPage {
  applicant : FormGroup;
  dobdate:any ;
  mark  = [
    {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad PropritorshipFirstPage');
  }
  isValidDate(date) {
    var temp = date.split('-');
  ...
  applicantForm()
  {
    ...
    localStorage.setItem('part1', JSON.stringify(this.applicant.value));    this.navCtrl.push('PropritorshipSecondPage');
  }
}
