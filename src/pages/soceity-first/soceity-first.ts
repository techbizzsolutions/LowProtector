...
@IonicPage()
@Component({
  selector: 'page-soceity-first',
  templateUrl: 'soceity-first.html',
})
export class SoceityFirstPage {
  applicant : FormGroup;
  title:any;
  dobdate:any ;
  mark  = [
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad SoceityFirstPage');
  ...
  applicantForm()
  {
   ...
   // this.applicant.value.dob = this.dobdate;
    ...
    localStorage.setItem('part1', JSON.stringify(this.applicant.value));    this.navCtrl.push('SoceitySecondPage',{name:this.navParams.data.name});
  }
