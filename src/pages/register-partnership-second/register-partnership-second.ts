...
@IonicPage()
@Component({
  selector: 'page-register-partnership-second',
  templateUrl: 'register-partnership-second.html',
})
export class RegisterPartnershipSecondPage {
  applicant : FormGroup;
  title:any;dobdate:any ;
  Trade  = [
    {
      title: 'Manufactures',
      selected: false
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad RegisterPartnershipSecondPage');
    this.title = this.navParams.data.name + " part 2";
  }
  applicantForm()
  {
    ...
    this.navCtrl.push('PaymentPage',{name:this.navParams.data.name});
    ...
    // this.senddata(this.navParams.data.name +" Part1: " + JSON.stringify(part1) + " "+this.navParams.data.name +" Part2: " +JSON.stringify(this.applicant.value));
